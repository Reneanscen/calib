...
from kpl_msgs.srv import StringSrv
...
def set_arm_mode(mode):
    # 等待 /manipulation/arm_mode 服务可用
    rospy.wait_for_service('/manipulation/arm_mode')
    try:
        # 创建服务代理
        arm_mode_service = rospy.ServiceProxy('/manipulation/arm_mode', StringSrv)
        # 调用服务并传递请求
        response = arm_mode_service(mode)
        # 返回服务的响应
        return response.success  # 假设响应有一个布尔类型的 success 字段
...
def calib_camera_base():
    ...
    # 调用函数并设置 arm 模式
    mode = "task_camera_calibration"  # 启动机械臂运行 // task_camera_calibration
    result = set_arm_mode(mode)
