...
def save_matrix_and_euler_angles(matrix, euler_angles, filename, precision=5):
    """
    将矩阵和欧拉角保存为 YAML 文件，确保输出的文档中没有多余的引号，并保留到指定的小数位。

    参数:
    - matrix (numpy.ndarray): 3x3 矩阵
    - euler_angles (numpy.ndarray): 欧拉角数组
    - filename (str): 保存的文件名
    - precision (int): 保留的小数位数，默认为5
    """
    # 将矩阵和欧拉角转换为列表，并保留到指定的小数位
    matrix_list = np.round(matrix, precision).tolist()
    euler_angles_list = np.round(euler_angles, precision).tolist()

    # 自定义 YAML 代表器，以确保列表不带引号
    ...
    data = {
        'matrix': matrix_list,
        'euler_angles_xyz': euler_angles_list
    }

    with open(filename, 'w') as file:
        yaml.dump(data, file, default_flow_style=False, allow_unicode=True)

    print(f"矩阵和欧拉角已保存为 {filename}")
...
class Camera2RobotOnBase:
    ...
    def camera2robotbase(self):
        ...
        images = glob.glob(str(ROOT)+ '/' +'calimageworld00_wt/*.jpg')
        import re
        ...
        T_camera2robotbase=T_camera2base
        ...
        # f=open(str(ROOT)+ '/' +"camera2robotbase4word.txt",'w')
        # f.write(str(T_camera2robotbase))
        # f.close()
        matrix_c2r = np.array(T_camera2robotbase)
        # 创建一个示例欧拉角（假设为 [roll, pitch, yaw]）
        euler_angles = np.array(euler_angles_deg)
        # 保存为 YAML 文件，保留到小数点后5位
        save_matrix_and_euler_angles(matrix_c2r, euler_angles, str(ROOT)+ '/' +'camera2robotbase4word_wt.yaml', precision=5)
